...
public:
    ...
    VkImageView createImageView(VkImage image, VkFormat format) {
        ...
        if (vkCreateImageView(mVkHack.GetVkDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
    ...
    void createTextureSampler() {
        ...
        vkGetPhysicalDeviceProperties(mVkHack.GetVkPhyDevice(), &properties);
        ...
        if (vkCreateSampler(mVkHack.GetVkDevice(), &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) {
   ...
   void createTextureImage()
   {
       ...
       void* data;
       vkMapMemory(mVkHack.GetVkDevice(), stagingBufferMemory, 0, imageSize, 0, &data);
   ...
   void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
       ...
       if (vkCreateImage(mVkHack.GetVkDevice(), &imageInfo, nullptr, &image) != VK_SUCCESS) {
           throw std::runtime_error("failed to create image!");
       }
   
       VkMemoryRequirements memRequirements;
       vkGetImageMemoryRequirements(mVkHack.GetVkDevice(), image, &memRequirements);
       ...
       if (vkAllocateMemory(mVkHack.GetVkDevice(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
           throw std::runtime_error("failed to allocate image memory!");
       }
   
       vkBindImageMemory(mVkHack.GetVkDevice(), image, imageMemory, 0);
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkUpdateDescriptorSets(mVkHack.GetVkDevice(), static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
        }
    }
