...
void ShadowMapInjectLightCS(uint2 ThreadID : SV_DispatchThreadID)
{
    ...
    if(PhotonTraverseOctree(VoxelSpaceIndex,NodeIndex,parentIndex,SubOctreeIndex))
    {
...
//IrradianceBrickBufferWOnly
//IrradianceBrickBufferROnly
//NodeCountAndOffsetBufferR
...
void GatherValuesFromLowLevelCS(uint3 groupId : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
    ...
    for ( uint i = 0; i < 8; i++ )
    {
        ...
        uint nodeFlag = SpaseVoxelOctreeR[GetFlagCoords( childIndex )];
        if ( nodeFlag == NODE_UNKOWN || ( nodeFlag & NODE_LIT ) == 0x0 )
            continue;
        ...
        for ( uint j = 0; j < 27; j++ )
        {
            ...
            brick[j] = IrradianceBrickBufferROnly.Load(int4(ChilebrickCoords + localOffset,0));
            IrradianceBrickBufferWOnly[ChilebrickCoords + localOffset] = brick[j];
        }
        ...
        for ( uint k = 0; k < CORNERS_COUNT; k++ )
        {
            ...
            Color += brick[index].xyz * ( 8 * 8 / 512.0f);
        }
        ...
        for ( uint l = 0; l < EDGES_COUNT; l++ )
        {
            ...
            Color += brick[index].xyz * (16 * 12 / 512.0f);
        }
        ...
        for ( uint m = 0; m < SIDES_COUNT; m++ )
        {
            ...
            Color += brick[index].xyz * (32 * 6 / 512.0f);
        }

        uint centerIndex = 1 + 1 * BRICK_SIZE + 1 * BRICK_SIZE * BRICK_SIZE;
        Color += brick[centerIndex].xyz * (64 * 1 / 512.0f);
