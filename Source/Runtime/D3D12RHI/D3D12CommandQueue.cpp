...
{
	ThrowIfFailed(
		device->GetDXDevice()->CreateFence(0, D3D12_FENCE_FLAG_NONE,
			IID_PPV_ARGS(&d3d12_fence)));

	//eventHandle = CreateEventEx(nullptr, NULL, CREATE_EVENT_INITIAL_SET, EVENT_ALL_ACCESS);
}
...
void XD3D12CommandQueue::ExecuteCommandListInteral(std::vector<XD3D12DirectCommandList>& Lists)
{
	...
	d3d12_cmd_queue->ExecuteCommandLists(Lists.size(), execute_cmd_queue.data());
